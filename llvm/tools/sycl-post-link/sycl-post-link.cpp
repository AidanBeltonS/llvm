...
static TableFiles processOneModule(std::unique_ptr<Module> M, bool IsEsimd,
                                   bool SyclAndEsimdCode, bool IsLLVMUsedRemoved) {
  TableFiles TblFiles;
  if (!M)
    return TblFiles;
...
static TableFiles processInputModule(std::unique_ptr<Module> M, bool RemovedLLVMUsed) {
  if (!SplitEsimd)
    return processOneModule(std::move(M), false, false, false);
  ...
  TableFiles SyclTblFiles =
      processOneModule(std::move(SyclModule), false, SyclAndEsimdCode, RemovedLLVMUsed);
  TableFiles EsimdTblFiles =
      processOneModule(std::move(EsimdModule), true, SyclAndEsimdCode, false);
